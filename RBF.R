...
RBF <- function(train_dataset, validate_dataset, non_normalize,neurons, predictor_order, learning_rate){
        library("RSNNS")
        train_input <- as.matrix(train_dataset[,1:predictor_order])
        train_output <- as.matrix(train_dataset[,predictor_order+1])
        test_input <- as.matrix(validate_dataset[,1:predictor_order])
        test_actual <- as.vector(validate_dataset[,predictor_order+1])
         
        data<-rbf(train_input,train_output,size = c(neurons) )
        result <- predict(data,test_input)
        result <- as.vector(result)
       
        result <- denormalized(result,non_normalize)
        test_actual <- denormalized(test_actual,non_normalize)
